...
/*	
  	@소프트웨어 테스트 정의 및 설명
	-소프트웨어 테스트 정의 : 시스템이나 시스템 구성 요소 또는 SW 프로그램을
	실행하고 평가하는 과정
	
	@테스트의 목적
	-개발 된 결과물을 검증 및 확인하여 최종 사용자에게 안정적이고 성능이 뛰어난
	소프트웨어를 제공하는 것
	...
	**테스트는 프로젝트 전반에 걸쳐서 수행됨**
	...
	소프트웨어 생명주기 종류
	1. 폭포수 모델
	특징:	소프트웨어 개발의 각 단계가 순차적으로 진행
		각 단계 종료 후 다음 단계 진행
		단순하거나 응용 분야를 잘 알고 있는 경우 적합한 모델
	단점:	소프트웨어 요구사항이 초기에 명확하게 정의되기 어려움
		소프트웨어가 개발 후반부에 구체화됨
		중요한 문제점들이 뒤늦게 발견됨
	...
	2.프로토타이핑 모델
	특징:	요구사항이 초기에 결정되면 개발단계에서 유지보수가 일어남
		요구사항이 불분명한 경우 유용한 모델
	단점:	고객은 시제품을 통해 구현 된 기능을 확인 가능함
		품질에 대한 확인 어려움
		시제품 작동을 데모하기 위해 적절하지 않은 프로그래밍 언어 및 비효율적인
		알고리즘이 사용 되는 경우가 많음
		
		-단계
		요구사항 수집 -> 프로토타입 설계 -> 프로토타입 개발 -> 고객 평가
		->프로토타입 정제 -> 요구사항 수집 -> 이 과정을 반복한다.
	
	@테스트 주도 개발(Test Driven Development)
	-정의
	테스트를 기반으로 개발을 진행하는 개발방법론
	1999년도 애자일 기반의 개발 방봅론이 나왔을 때 소개됨
	...
	-> 코드 작성 -> 테스트 수행 -> 모든 테스트 성공 할 때까지 수행
	-> 코드 리팩토링 -> 반복
*/	
//	@테스트 주도 개발 절차별 세부사항
//	1.테스트추가
...
//	테스트 케이스 작성 : jUnit과 같은 유닛 테스트를 활용
//	
//	2.테스트 수행
//	추가한 테스트 케이스를 포함하여 유닛 테스트 수행
//	새롭게 추가된 테스트 케이스 결과 : 실패(fail)
//	새롭게 추가 된 테스트 케이스에 대한 코드
//	-> 구현되지 않았기 때문에 실패
//	추가 된 테스트 케이스만 실패
...
//	목표 : 테스트 케이스 결과가 성공(Pass)할 수준의 코드 작성
//	리팩토링 단계 : 코드 개선 기회
//	
//	4.테스트 수행
//	테스트 케이스를 다시 한 번 수행
//	목표 : 추가 된 테스트 케이스 및 모든 테스트 케이스 통과(Pass)
//	모두 통과 시
//	-> 개발자 : 구현사항이 요구사항을 만족하는 것을 확인 가능
//	-> 새롭게 추가한 코드가 부작용을 일으키지 않았단느 것을 확인 가능
//	테스트 케이스가 통과하지 않는 경우
//	->추가 된 코드에 문제 있음 -> 수정 필요
//	수정 후 이 단계를 다시 수행
//	
//	5. 코드 리팩토링
...
//	@Test안에 테스트 메서드를 만듬
//	실제 업무 클래스 생성 -> 로직은 실패해야함
//	실패한 코드를 확인 후 기능이 돌아가는 코드를 구현할
//	테스트 동장 수행(성공 할 때까지 위 내용 반복)
//	성공했는데 리팩토링이 필요하면 수행
//	위 단계들을 계속 수행
//	
//	@jUnit 동작 준비 단계
...
//	boundary value
//	
//	입력 조건의 중간 값 보다는 경계 값에 오류(error)가 발생 할 확률이 경험적으로
//	높다는 점을 감안하여 테스트 케이스를 설계하는 방법
//	
//	ex:1~100까지의 임의의 숫자
//	
//	1, 100 두개를 통과하면 성공으로 치자
//	
//	0,1,2 / 99,100,101
//	숫자에서 +- 1씩은 하자
