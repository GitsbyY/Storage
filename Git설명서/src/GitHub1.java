...
/*
	...
	Git을 사용 할 때 소스코드를 올려놓는 서버가 필요한데,
	이 서버를 제공해주는 서비스
	...
	@Git의 테이터 상태 관리
	-Modified : 수정한 파일을 아직 로컬 테이터에 적용하지 않은 상태
	...
	3)Directory
	-원격저장소에 저장 된 소스코드와 그 변경사항을 관리하기 위해
	내 컴퓨터에 변경점에 대한 정보만을 관리
	
	4)Branch
	-소프트웨어 개발에서 다양한 버전이나 특정 기능들을 따로 구현해야 할 때
	...
	사용하기 위해서 개인 계정을 등록해야하며,
	인터넷에 연결되어 있어야 한다
	
	즉, githubsms git 소프트웨어를 지원하는 일종의 클라우드 서비스이다
	
	git에서 제공하지 않는 기능도 추가적으로 제공하고 있다.
	ex : UI 제공
	...
	@소스코드 관리 서버
	-git 시스템을 사용하여 발생되는 소스코드의 변경 정보와 모든 소스코드 파일을
	저장하기 위한 공간
	-git을 사용하는데 있어 개인적으로 서버를 구성하여 프로젝트 관리도 가능
	-git hub 이용 비용
		오픈소스 프로젝트 : 무료
		사용 private 프로젝트 : 유료
	-소스코드 관리서버 기능
	...
	@Git Hub의 커뮤니티 기능
	-Git Hub 프로젝트 소개 홈페이지로도 활용 가능
	-Git Hub 사이트에서 프로젝트 페이지에 접속 시 현재 소스코드들이 보여짐
		누구나 문서를 생성할 수 있도록 제공됨
	-질문과 답변 가능
	-이슈를 등록 및 상태 체크
	...
	-개발진행 관리 기능
		개발 진행 관리와 차기 버전에서 개선해야 할 부분 정리에 큰 도움
