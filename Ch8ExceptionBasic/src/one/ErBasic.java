...
/*		
		...
		글자 그대로의 의미이며 이외에도 '논리적 에러' (logical error)가 있는데
		...
		컴파일러가 알려준 에러들은 모두 수정해서 컴파일을 성공적으로 마쳐야 한다
		클래스 파일(*.class)가 생성되고, 생성 된 클래스 파일을 실행 할 수 있게 된다
		
		하지만, 컴파일을 에러없이 성공적으로 마쳤다고 해서 프로그램의 실행 시에도 에러가
		발생하지 않는 것은 아니다
		
		실행 도중에 발생할 수 있는 잠재적인 오류까지 검사할 수 없기 때문에 비정상적으로 종료 될 수 있다.
		...
		자바에서 실행 시 발생 할 수 있는 프로그램 오류를 에러와 예외,
		두 가지로 구분한다
				
		에러(Error)는 메모리 부족(OutOfMemory)이나
